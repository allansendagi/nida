...
export async function POST(request: Request, { params }: Props) {
  try {
    ...
    // Get the negotiation with full details
    ...
    // Check offer state - must be 'offered' to accept
    if (negotiation.offer_state !== 'offered') {
      return NextResponse.json(
        { error: `Cannot accept: this offer is in '${negotiation.offer_state}' state` },
        { status: 400 }
      );
    }
    ...
    // Update negotiation state and offer state
    ...
    // Update intent status and clear offer tracking
    ...
    console.error('Error in POST /api/negotiations/[id]/accept:', error);
    return NextResponse.json(
